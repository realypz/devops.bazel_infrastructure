...
{
    static auto& dereference(const P& p) noexcept
    {
        return *std::to_address(p);
    }

    using target_type = std::remove_pointer_t<decltype(std::to_address(std::declval<const P&>()))>;
};

template <class T>
struct ptr_traits<T*> : applicable_traits
{
    static T& dereference(T* p) noexcept
    {
        return *p;
    }
...
concept invocable_dispatch_ptr =
    ...
    invocable_dispatch<D, const P, NE, R, Args...> || invocable_dispatch<D, void, NE, R, Args...>;
...
R invocation_dispatcher_ref(const char* self, Args... args) noexcept(
    is_invoker_well_formed<F, typename ptr_traits<P>::target_type, true, R, Args...>())
{
    return invoke_dispatch<F, R>(
        ptr_traits<P>::dereference(*std::launder(reinterpret_cast<const P*>(self))),
...
template <class P>
void copying_dispatcher(char* self,
                        const char* rhs) noexcept(has_copyability<P>(constraint_level::nothrow))
{
...
template <class P>
void destruction_dispatcher(char* self) noexcept(has_destructibility<P>(constraint_level::nothrow))
{
...
struct overload_traits_impl : applicable_traits
{
    ...
    struct meta_provider
    {
        ...
        {
            if constexpr (invocable_dispatch<D, typename ptr_traits<P>::target_type, NE, R,
                                             Args...>)
            {
                return &invocation_dispatcher_ref<
                    P, typename D::template invoker<typename ptr_traits<P>::target_type>, R,
                    Args...>;
            }
            else if constexpr (invocable_dispatch<D, const P, NE, R, Args...>)
            {
                return &invocation_dispatcher_ptr<P, typename D::template invoker<const P>, R,
                                                  Args...>;
            }
            else
            {
                return &invocation_dispatcher_void<typename D::template invoker<void>, R, Args...>;
            }
        }
    };
    ...
    static constexpr bool applicable_ptr = invocable_dispatch_ptr<D, P, NE, R, Args...>;
...
template <class R, class... Args>
struct overload_traits<R(Args...) noexcept> : overload_traits_impl<true, R, Args...>
{};
...
struct dispatch_traits_impl<D, Os...> : applicable_traits
{
  ...
  public:
    using meta =
        composite_meta<dispatcher_meta<typename overload_traits<Os>::template meta_provider<D>>...>;
    template <class... Args>
...
struct dispatch_traits<D> : instantiated_t<dispatch_traits_impl, typename D::overload_types, D>
{};
...
struct facade_traits<F> : instantiated_t<facade_traits_impl, typename F::dispatch_types, F>
{};
...
struct meta_ptr
{
    ...
    template <class P>
    constexpr explicit meta_ptr(std::in_place_type_t<P>) noexcept : ptr_(&storage<P>)
    {}
...
class proxy : public details::facade_traits<F>::base
{
    ...
    static constexpr bool HasNothrowPolyConstructor =
        std::conditional_t<proxiable<P, F>, std::is_nothrow_constructible<P, Args...>,
    ...
    template <class P, class... Args>
    static constexpr bool HasPolyAssignment = HasPolyConstructor<P, Args...> && HasDestructor;
    ...
    static constexpr bool HasCopyAssignment =
        HasNothrowCopyAssignment || (HasCopyConstructor && HasMoveConstructor && HasDestructor);
  ...
  public:
    ...
    explicit proxy(std::in_place_type_t<P>, std::initializer_list<U> il, Args&&... args) noexcept(
        HasNothrowPolyConstructor<P, std::initializer_list<U>&, Args...>)
    ...
    requires(!HasNothrowPolyAssignment<std::decay_t<P>, P> && HasPolyAssignment<std::decay_t<P>, P>)
    {
    ...
    decltype(auto) invoke(Args&&... args) const noexcept(HasNothrowInvocation<D, Args...>)
    requires(Traits::template has_dispatch<D> && requires { typename MatchedOverload<D, Args...>; })
    {
        return meta_->template dispatcher_meta<typename details::overload_traits<
            MatchedOverload<D, Args...>>::template meta_provider<D>>::dispatcher(ptr_,
                                                                                 std::forward<Args>(
                                                                                     args)...);
    }
...
class inplace_ptr
{
  public:
    template <class... Args>
    inplace_ptr(Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args...>)
...
proxy<F> make_proxy_impl(Args&&... args)
{
    ...
    {
        return proxy<F>{std::in_place_type<inplace_ptr<T>>, std::forward<Args>(args)...};
    }
    else
    {
...
proxy<F> make_proxy_inplace(Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args...>)
{
    return proxy<F>{std::in_place_type<details::inplace_ptr<T>>, std::forward<Args>(args)...};
}

template <facade F, inplace_proxiable_target<F> T, class U, class... Args>
proxy<F> make_proxy_inplace(std::initializer_list<U> il, Args&&... args) noexcept(
    std::is_nothrow_constructible_v<T, std::initializer_list<U>&, Args...>)
{
    return proxy<F>{std::in_place_type<details::inplace_ptr<T>>, il, std::forward<Args>(args)...};
}

template <facade F, class T>
proxy<F> make_proxy_inplace(T&& value) noexcept(std::is_nothrow_constructible_v<std::decay_t<T>, T>)
...
struct flat_reduction<std::tuple<Os...>, I> : std::type_identity<std::tuple<Os..., I>>
{};
...
template <class Ds = std::tuple<>, proxiable_ptr_constraints C = relocatable_ptr_constraints,
          class R = void>
    ...
    {                                                                                              \
      private:                                                                                     \
        ...
        {                                                                                          \
            ...
            decltype(auto) operator()(__Args&&... __args) ___PRO_DIRECT_FUNC_IMPL(__DEFEXPR)       \
        };                                                                                         \
                                                                                                   \
      public:                                                                                      \
        ...
        {                                                                                          \
            ...
            decltype(auto) __NAME(__Args&&... __args) const ___PRO_DIRECT_FUNC_IMPL(               \
                (static_cast<::pro::details::dependent_t<const __P*, __Args...>>(this)             \
                     ->template invoke<__Name>(::std::forward<__Args>(__args)...)))                \
        };                                                                                         \
    }
#define PRO_DEF_MEMBER_DISPATCH_WITH_DEFAULT(__NAME, __FUNC, __DEFFUNC, ...)                       \
    ___PRO_DEF_DISPATCH_IMPL(__NAME, __self.__FUNC(::std::forward<__Args>(__args)...),             \
                             __DEFFUNC(::std::forward<__Args>(__args)...), __VA_ARGS__)

#define PRO_DEF_FREE_DISPATCH_WITH_DEFAULT(__NAME, __FUNC, __DEFFUNC, ...)                         \
    ___PRO_DEF_DISPATCH_IMPL(__NAME, __FUNC(__self, ::std::forward<__Args>(__args)...),            \
                             __DEFFUNC(::std::forward<__Args>(__args)...), __VA_ARGS__)

#define PRO_DEF_MEMBER_DISPATCH(__NAME, ...)                                                       \
    PRO_DEF_MEMBER_DISPATCH_WITH_DEFAULT(__NAME, __NAME, ::pro::details::invalid_call, __VA_ARGS__)

#define PRO_DEF_FREE_DISPATCH(__NAME, __FUNC, ...)                                                 \
    PRO_DEF_FREE_DISPATCH_WITH_DEFAULT(__NAME, __FUNC, ::pro::details::invalid_call, __VA_ARGS__)
