...
def _create_clang_config_base(llvm_dir, llvm_major_version, sysroot):
    """ Function: Returns a dictionary with the configuration for the clang toolchain. Suits for clang on both Linux and macOS.
...
def MACRO_llvm_toolchain_config(name, llvm_dir, llvm_major_version, sysroot):
    ...
    """
    base_config = _create_clang_config_base(llvm_dir, llvm_major_version, sysroot)
